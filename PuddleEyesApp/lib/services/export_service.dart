...
class ExportService {
  static Future<String> exportGPX() async {
    ...
    for (var obs in TrackingService.getAllObstacles()) {
      final wpt =
          Wpt(lat: obs.lat, lon: obs.lon, ele: obs.depth, time: obs.timestamp);
      gpx.wpts.add(wpt);
    }
    ...
    final file = File(
        '${dir.path}/puddleeyes_track_${DateTime.now().millisecondsSinceEpoch}.gpx');
  ...
  static Future<String> exportKML() async {
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File(
        '${dir.path}/puddleeyes_track_${DateTime.now().millisecondsSinceEpoch}.kml');
